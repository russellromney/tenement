...
//! Supports per-process wildcard certificates for subdomain routing.
...
/// DNS provider environment variable names
pub fn dns_token_env_var(provider: &str) -> &'static str {
    match provider {
        "cloudflare" => "CF_API_TOKEN",
        "route53" => "AWS_ACCESS_KEY_ID", // Also needs AWS_SECRET_ACCESS_KEY
...
/// Generate Caddyfile with per-process wildcard certificates
///
/// This generates separate wildcard blocks for each process, enabling
/// instance routing like `prod.api.example.com` via DNS-01 challenge.
///
/// Note: Each wildcard requires a separate certificate from Let's Encrypt.
/// DNS-01 challenge requires DNS provider API access.
pub fn generate_caddyfile_with_processes(
    domain: &str,
    port: u16,
    processes: &[String],
    dns_provider: &str,
) -> String {
    let token_env = dns_token_env_var(dns_provider);

    let mut output = format!(
        r#"# Tenement reverse proxy with per-process wildcards
# Generated by: ten caddy --domain {domain} --port {port} --dns-provider {dns_provider}
#
# Features:
# - Automatic HTTPS via Let's Encrypt
# - Single-level wildcard: *.{domain} (e.g., api.{domain})
# - Per-process wildcards: *.process.{domain} (e.g., prod.api.{domain})
#
# IMPORTANT: DNS-01 challenge requires DNS provider API access.
# Set environment variable: {token_env}
#
# For Cloudflare:
...
# Single-level wildcard for process routing (api.{domain}, web.{domain})
# Uses TLS-ALPN-01 challenge (no DNS required)
*.{domain} {{
    reverse_proxy localhost:{port}
    encode gzip zstd
}}
"#
    ...
    // Add per-process wildcard blocks with DNS-01 challenge
    if !processes.is_empty() {
        output.push_str(&format!(
            r#"
# Per-process wildcards for instance routing
# Requires DNS-01 challenge (DNS provider: {dns_provider})
"#
        ));
        ...
        for process in processes {
            output.push_str(&format!(
                r#"
# Instance routing for {process}: e.g., prod.{process}.{domain}
*.{process}.{domain} {{
    reverse_proxy localhost:{port}
    encode gzip zstd
    tls {{
        dns {dns_provider} {{env.{token_env}}}
    }}
}}
"#
            ));
...
pub fn run(
    ...
    // Generate Caddyfile - use per-process wildcards if dns_provider is specified
    let caddyfile = if let Some(ref provider) = dns_provider {
        // Load config to get process names
        let processes = match Config::load() {
            Ok(config) => config.service.keys().cloned().collect::<Vec<_>>(),
            Err(_) => {
                // No config file found, just use basic wildcard
                tracing::warn!("No tenement.toml found - generating basic Caddyfile without per-process wildcards");
                vec![]
            }
        };
    ...
    match output {
        ...
        None => {
            ...
            if let Some(ref provider) = dns_provider {
                let token_env = dns_token_env_var(provider);
                println!();
                println!("# DNS-01 Setup Required for Per-Process Wildcards:");
                println!("# Set environment variable before starting Caddy:");
                println!("#   export {}=\"your-api-token\"", token_env);
                println!("#");
                println!("# Then start Caddy with: caddy run --config Caddyfile");
            }
            return Ok(());
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_dns_token_env_var_cloudflare() {
        assert_eq!(dns_token_env_var("cloudflare"), "CF_API_TOKEN");
    }

    #[test]
    fn test_dns_token_env_var_route53() {
        assert_eq!(dns_token_env_var("route53"), "AWS_ACCESS_KEY_ID");
    }

    #[test]
    fn test_dns_token_env_var_digitalocean() {
        assert_eq!(dns_token_env_var("digitalocean"), "DO_AUTH_TOKEN");
    }

    #[test]
    fn test_dns_token_env_var_unknown() {
        assert_eq!(dns_token_env_var("unknown_provider"), "DNS_API_TOKEN");
    }
    ...
    fn test_generate_caddyfile_with_processes_basic() {
        ...
        // Check per-process wildcards
        assert!(caddyfile.contains("*.api.example.com {"));
        assert!(caddyfile.contains("*.web.example.com {"));

        // Check DNS-01 configuration
        assert!(caddyfile.contains("dns cloudflare"));
        assert!(caddyfile.contains("{env.CF_API_TOKEN}"));
    }
    ...
    fn test_generate_caddyfile_with_processes_empty() {
        ...
        // Should NOT have per-process wildcards
        assert!(!caddyfile.contains("*.api.example.com"));
    }
    ...
    fn test_generate_caddyfile_with_processes_route53() {
        ...
        assert!(caddyfile.contains("{env.AWS_ACCESS_KEY_ID}"));
    }
